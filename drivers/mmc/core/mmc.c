...
static const struct mmc_fixup mmc_fixups[] = {
	/*
	 * There is a bug in some HYNIX emmc chips where the wear leveling
	...
	MMC_FIXUP_REV("VYL00M", 0x15, CID_OEMID_ANY,
		      ...
		      add_quirk_mmc, MMC_QUIRK_HYNIX_WL_PATCH),
	MMC_FIXUP_REV("KYL00M", 0x15, CID_OEMID_ANY,
		      cid_rev(0, 0x25, 1997, 1), cid_rev(0, 0x25, 2012, 12),
		      add_quirk_mmc, MMC_QUIRK_HYNIX_WL_PATCH),
	MMC_FIXUP_REV("XYNIH", 0x90, CID_OEMID_ANY,
		      cid_rev(0, 0x25, 1997, 1), cid_rev(0, 0x25, 2012, 12),
		      add_quirk_mmc, MMC_QUIRK_HYNIX_WL_PATCH),
	END_FIXUP
};
	...
	struct mmc_card *oldcard)
{
	...
	/*
	 * Patch the firmware in certain Hynix emmc chips to fix a
	 * wear leveling bug.
	 */
	if (card->quirks & MMC_QUIRK_HYNIX_WL_PATCH)
		mmc_fixup_hynix_fw(card);
