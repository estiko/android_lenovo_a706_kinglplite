...
/*
 * Quirk code to fix bug in wear leveling firmware for certain hynix emmc
 * chips
 */
...
void mmc_fixup_hynix_fw(struct mmc_card *card)
{
	int err;
	...
	if (err)
		pr_err("%s : Failed to fixup hynix emmc firmware(%d)\n",
